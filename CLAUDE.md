...
公開モデルフィルタリング: dbt モデルの YAML で `meta.public: true` を設定したモデルのみ metadata.json に含まれる。
...
2. dbt の manifest.json / catalog.json からモデル情報を抽出
3. `meta.public: true` のモデルのみフィルタリング
4. lineage (DAG) を構築
5. dist/metadata.json に出力
