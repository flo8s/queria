...
ビルドすると `.fdl/` 内に以下のファイルが配置される:

```
datasets/my_city/.fdl/
...
```bash
duckdb "ducklake:datasets/my_city/.fdl/ducklake.duckdb" \
...
```bash
cat datasets/my_city/.fdl/metadata.json | python -m json.tool
```
